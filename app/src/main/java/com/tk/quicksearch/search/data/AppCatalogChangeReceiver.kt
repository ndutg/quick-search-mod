 ...
 * This receiver covers package changes that happen while Quick Search is not in memory, so a
 * stale catalog is never restored on the next launch.
 */
class AppCatalogChangeReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        AppCache(context.applicationContext).clearCache()
    }
}
